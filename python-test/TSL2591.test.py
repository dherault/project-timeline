...
while True:
    ...
    if lux != 0:
      print("Total light: {0}lux, {1}".format(lux, datetime.datetime.now()))
    ...
    if infrared != 0:
      print("Infrared light: {0}, {1}".format(infrared, datetime.datetime.now()))

    # Visible-only levels range from 0-2147483647 (32-bit)
    visible = sensor.visible

    if visible != 0:
      print("Visible light: {0}, {1}".format(visible, datetime.datetime.now()))
